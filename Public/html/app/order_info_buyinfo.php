<?php
    include 'inc/top.php';
?> 

<div class="cont">
	<div class="tit"><a href="#" onclick="window.history.go(-1);"><返回</a>订单详情</div>
	<hr>
	<div class="order_info"><label>订单号: B20183225485965</label><label>[已生效]</label></div>
	<div class="order_info"><label><span><b>需支付：85,084.5</b></span>元</label><br/>其中：待付货款 <span><b>84,500</b></span>元，待付利息 <span><b>84.5</b></span>元，请按合同说明方式付款至平台指定帐号。</div>
	<div class="order_info" onclick="location.href='buyinfo_bill.php'"><label>玉米　2018年　辽宁　一等　容重720g/l</label><br/><label><b>1,700</b>元/吨　　（查看）</label></div>
	<div class="order_item" onclick="location.href='buyinfo_item.php'">
		&#8470;.1021112<label><span><b>[待确认收货]</b></span>　(详情)</label><br/>2箱　净重50吨<label>85,000元　结息85元(5天)</label>
	</div>
	<div class="order_item" onclick="location.href='buyinfo_item.php'">
		&#8470;.1021123<label><span><b>[待确认收货]</b></span>　(详情)</label><br/>2箱　净重50吨<label>85,000元　结息85元(5天)</label>
	</div>
	<div class="order_info"></div>
	<div class="order_item">货款合计：<label><b>170,000</b> 元</label><br/>&nbsp;<label>(首付<b>34,000</b>元，尾款<b>136,000</b>元，日利0.0002)</label></div>
	<div class="order_item">收货方式：物流到门<label>运费：<b>3,600</b>元</label><br/>收货地址：辽宁省沈阳市和平区xx街道xx号<br/>联系人：赵钱孙　　手机号：13910533527</div>
	<div class="order_info"></div>
	<div class="order_item">已付尾款：<b>85,000</b>元<label>已付利息：<b>85</b>元</label></div>
	<div class="order_item">待付尾款：<b>169,000</b>元<label>尾款计息：<b>85</b>元</label></div>
	<div class="order_info"><<a href="">查看电子合同</a>></div>
	<label>成交时间：2018/07/19 12:21:35</label>
</div>
<?php
    include 'inc/bot.php';
?>